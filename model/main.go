...
func migrateClickHouseLogDB() error {
	...
	if err := LOG_DB.Exec(clickHouseConversationLogCreateTableSQL(ttlDays)).Error; err != nil {
		return err
	}
...
func clickHouseConversationLogCreateTableSQL(ttlDays int) string {
	return fmt.Sprintf(`
CREATE TABLE IF NOT EXISTS conversation_logs (
	...
	use_time Int32 DEFAULT 0,
	is_stream UInt8 DEFAULT 0,
	request_preview String DEFAULT '',
	response_preview String DEFAULT '',
...
func syncClickHouseLogTTL(ttlDays int) error {
	...
	if expression != "" {
		...
		return LOG_DB.Exec("ALTER TABLE conversation_logs MODIFY TTL " + expression).Error
	}
	...
	if !hasTTL {
		return syncClickHouseConversationLogTTL()
	}
...
func syncClickHouseConversationLogTTL() error {
	var createTableSQL string
	if err := LOG_DB.Raw("SHOW CREATE TABLE conversation_logs").Scan(&createTableSQL).Error; err != nil {
		return err
	}
	if !clickHouseCreateTableHasTTL(createTableSQL) {
		return nil
	}
	return LOG_DB.Exec("ALTER TABLE conversation_logs REMOVE TTL").Error
}
