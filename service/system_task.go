...
func runLogCleanupTask(ctx context.Context, task *model.SystemTask, runnerID string) {
	...
	for {
		...
		conversationRemaining, err := model.CountOldConversationLog(ctx, payload.TargetTimestamp)
		if err != nil {
			failSystemTask(task, runnerID, err)
			return
		}
		...
		for state.Remaining > 0 {
			...
			if rowsAffected == 0 {
				rowsAffected, err = model.DeleteOldConversationLogBatch(ctx, payload.TargetTimestamp, payload.BatchSize)
				if err != nil {
					failSystemTask(task, runnerID, err)
					return
				}
