...
import type { ConversationLog, LogCategory } from '../types'
...
function ConversationLogsCard<TData>({
  cells,
}: {
  cells: Map<string, Cell<TData, unknown>>
}) {
  ...
  return (
    <div className='space-y-2.5'>
      ...
      <div className='grid grid-cols-2 gap-1.5'>
        ...
        <SummaryField label={t('Tokens')} cell={cells.get('prompt_tokens')} />
        <SummaryField label={t('Cost')} cell={cells.get('quota')} />
        <SummaryField
          label={t('Request')}
          cell={cells.get('request_preview')}
          className='col-span-2'
        />
        <SummaryField
          label={t('Response')}
          cell={cells.get('response_preview')}
          className='col-span-2'
        />
        {rowData?.error_message && (
          <div className='bg-muted/20 col-span-2 min-w-0 rounded-md px-2 py-1.5'>
            ...
            <div className='text-destructive line-clamp-2 text-xs break-words'>
              {rowData.error_message}
            </div>
          </div>
        )}
        ...
        return (
          ...
          >
            ...
            {logCategory === 'conversation' && (
              <ConversationLogsCard cells={cells} />
            )}
