 ...
 */
export type LogCategory = 'common' | 'conversation' | 'drawing' | 'task'
...
/**
 * Conversation logs specific filters
 */
export interface ConversationLogFilters extends CommonFilters {
  model?: string
  token?: string
  group?: string
  username?: string
...
export interface GetLogsResponse {
  ...
  message?: string
  data?: {
    items: UsageLog[] | ConversationLog[] | MidjourneyLog[] | TaskLog[]
    total: number
    page: number
    page_size: number
  }
...
export interface GetConversationLogsParams {
  p?: number
  page_size?: number
  username?: string
  token_name?: string
