...
function buildConversationApiParams(config: {
  page: number
  pageSize: number
  searchParams: Record<string, unknown>
  columnFilters?: Array<{ id: string; value: unknown }>
  ...
  const params: GetConversationLogsParams = {
    p: page,
    page_size: pageSize,
    ...(searchParams.model ? { model_name: String(searchParams.model) } : {}),
    ...(searchParams.token ? { token_name: String(searchParams.token) } : {}),
...
export async function fetchLogsByCategory(
  ...
  if (logCategory === 'conversation') {
    const params = buildConversationApiParams({
      page,
      pageSize,
      searchParams,
      columnFilters,
    ...
    return isAdmin
      ? await getAllConversationLogs(params)
      : await getUserConversationLogs(params)
  }
