...
const logSettingsSchema = z.object({
  ...
  ConversationLogMaxBodyBytes: z.coerce
    .number()
    .int()
    .min(1024)
    .max(10 * 1024 * 1024),
...
type LogSettingsSectionProps = {
  ...
  defaultConversationMaxBodyBytes: number
}
...
export function LogSettingsSection({
  ...
  defaultConversationMaxBodyBytes,
  ...
  const form = useForm<LogSettingsFormValues>({
    ...
    defaultValues: {
      ...
      ConversationLogEnabled: defaultConversationEnabled,
      ConversationLogMaxBodyBytes: defaultConversationMaxBodyBytes,
    },
  })
  ...
  const onSubmit = async (values: LogSettingsFormValues) => {
    const updates = [
      values.LogConsumeEnabled !== defaultEnabled
        ? { key: 'LogConsumeEnabled', value: values.LogConsumeEnabled }
        : null,
      values.ConversationLogEnabled !== defaultConversationEnabled
        ? {
            key: 'ConversationLogEnabled',
            value: values.ConversationLogEnabled,
          }
        : null,
      values.ConversationLogMaxBodyBytes !== defaultConversationMaxBodyBytes
        ? {
            key: 'ConversationLogMaxBodyBytes',
            value: values.ConversationLogMaxBodyBytes,
          }
        : null,
    ...
    <SettingsSection title={t('Log Maintenance')}>
      <Form {...form}>
        <SettingsForm onSubmit={form.handleSubmit(onSubmit)}>
            ...
            render={({ field }) => (
              <SettingsSwitchItem>
                <SettingsSwitchContent>
                  ...
                  <FormDescription>
                    {t(
                      'Store request and response bodies for model conversations so admins can review full dialogue content.'
                    )}
                  </FormDescription>
                </SettingsSwitchContent>
                <FormControl>
          ...
          <FormField
            control={form.control}
            name='ConversationLogMaxBodyBytes'
            render={({ field }) => (
              <div data-settings-form-span='full' className='grid gap-2'>
                <FormLabel>{t('Conversation body limit')}</FormLabel>
                <FormControl>
                  <Input
                    type='number'
                    min={1024}
                ...
                <FormDescription>
                  {t(
                    'Maximum bytes stored for each request or response body. Longer content is truncated.'
                  )}
                </FormDescription>
                <FormMessage />
              </div>
